...
namespace pic_handling
{
    ...
    void image_processing::main_loop()
    {
        ...
        printf("Begining the vision based algorithm for environment filtering\n");
        ...
        while (ros::ok())
        {
            if (icCamGrabber1.getbDepthReceived() && icCamGrabber1.getbColorReceived() && !bIsProcessing)
            {
                try
                {
                    ...
                    // listener.waitForTransform("/rs_d435_cam_color_optical_frame", "/tcp", ros::Time(0), ros::Duration(10.0) );
                    // listener.lookupTransform("/rs_d435_cam_color_optical_frame", "/link_1",ros::Time(0), base_0);
                    // listener.lookupTransform("/rs_d435_cam_color_optical_frame", "/link_3",ros::Time(0), forearm);
                    // listener.lookupTransform("/rs_d435_cam_color_optical_frame", "/link_2",ros::Time(0), shoulder);
                    // listener.lookupTransform("/rs_d435_cam_color_optical_frame", "/link_4",ros::Time(0), wrist_1);
                    // listener.lookupTransform("/rs_d435_cam_color_optical_frame", "/link_5",ros::Time(0), wrist_2);
                    // listener.lookupTransform("/rs_d435_cam_color_optical_frame", "/link_6",ros::Time(0), wrist_3);
                    // listener.lookupTransform("/rs_d435_cam_color_optical_frame", "/tcp",ros::Time(0), tool_center_point);
                }
            ...
            if (bColorStored1 && bDepthStored && !bIsProcessing)
            {
                bIsProcessing = true;
                bColorStored1 = false;
                bDepthStored = false;
     ...
     **********************************************************/
    image_processing::image_processing(ros::NodeHandle nh) : icCamGrabber1("1", true)       //,  icCamGrabber2("2", true)
    {
        _nh = nh;
    }
